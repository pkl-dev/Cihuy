...
$where = "WHERE a.id_siswa IS NOT NULL";
if ($search) {
    $where .= " AND (
...
$data = mysqli_query($conn, "
    SELECT a.*, s.nama AS nama_siswa, e.nama_ekskul 
    FROM absensi a
    JOIN siswa s ON a.id_siswa = s.id_siswa
    JOIN ekskul e ON a.id_ekskul = e.id_ekskul
    $where
    ...
    <table>
        ...
        <tbody>
            <?php if (mysqli_num_rows($data) > 0): ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="5" style="text-align:center;">Tidak ada data absensi siswa.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
